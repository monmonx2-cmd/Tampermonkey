diff --git a/suno一括入力.user.js b/suno一括入力.user.js
new file mode 100644
index 0000000000000000000000000000000000000000..7ba716465b2291850aa72b0bc91407f4ad6b1553
--- /dev/null
+++ b/suno一括入力.user.js
@@ -0,0 +1,1750 @@
+// ==UserScript==
...
+  }
+
+  function getRememberedGenderSelection(root) {
+    if (!root || !(root instanceof HTMLElement)) return '';
+    return root.dataset.tmLastGenderSelection || '';
+  }
+
+  function rememberGenderSelection(root, gender) {
+    if (!root || !(root instanceof HTMLElement) || !gender) return;
+    root.dataset.tmLastGenderSelection = gender;
+  }
+
...
+
+    if (root) {
+      const selectionStateBeforeClick = readGenderSelectionState(root, gender);
+      if (selectionStateBeforeClick === 'selected') {
+        rememberGenderSelection(root, gender);
+        return { mode: 'ui', ok: true, message: `ボーカル性別UIで ${gender} は既に選択済み` };
+      }
+
+      if (getRememberedGenderSelection(root) === gender) {
+        return { mode: 'ui', ok: true, message: `ボーカル性別UIで ${gender} は前回適用済み（再クリックをスキップ）` };
+      }
+
...
+          rememberGenderSelection(root, gender);
+          return { mode: 'ui', ok: true, message: `ボーカル性別UIで ${gender} を選択` };
+        }
+
+        rememberGenderSelection(root, gender);
+        return { mode: 'ui', ok: true, message: `ボーカル性別UIで ${gender} の選択を試行（同じ指定では再クリックしません）` };
+      }
+
